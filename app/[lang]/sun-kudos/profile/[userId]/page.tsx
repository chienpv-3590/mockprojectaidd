...
export default async function ProfilePage({ params }: { params: Promise<{ userId: string }> }) {
  ...
  // Redirect viewer to their own profile page if they hit their own ID.
  if (userId === user.id) redirect("/sun-kudos/profile");

  const years = await getUserKudosYears(supabase, userId);
  const initialYear = years[0] ?? new Date().getFullYear();

  const [notifications, unreadCount, profile, profileStats, heroRank, feedResult] =
    await Promise.all([
