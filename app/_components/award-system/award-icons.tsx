"use client";

import { Medal } from "@phosphor-icons/react";

const COLOR = "#FFFFFF";

// MM_MEDIA_Target — verbatim SVG from MoMorph design (component 214:1808):
// outer broken circle + inner broken circle + arrow piercing into the bullseye.
export function TargetIcon({ size = 24 }: { size?: number }) {
  return (
    <svg
      width={size}
      height={size}
      viewBox="0 0 20 20"
      fill={COLOR}
      xmlns="http://www.w3.org/2000/svg"
      aria-hidden
    >
      <path d="M10 0C8.68678 0 7.38642 0.258658 6.17317 0.761205C4.95991 1.26375 3.85752 2.00035 2.92893 2.92893C1.05357 4.8043 0 7.34784 0 10C0 12.6522 1.05357 15.1957 2.92893 17.0711C3.85752 17.9997 4.95991 18.7362 6.17317 19.2388C7.38642 19.7413 8.68678 20 10 20C12.6522 20 15.1957 18.9464 17.0711 17.0711C18.9464 15.1957 20 12.6522 20 10C20 8.84 19.79 7.69 19.39 6.61L17.79 8.21C17.93 8.8 18 9.4 18 10C18 12.1217 17.1571 14.1566 15.6569 15.6569C14.1566 17.1571 12.1217 18 10 18C7.87827 18 5.84344 17.1571 4.34315 15.6569C2.84285 14.1566 2 12.1217 2 10C2 7.87827 2.84285 5.84344 4.34315 4.34315C5.84344 2.84285 7.87827 2 10 2C10.6 2 11.2 2.07 11.79 2.21L13.4 0.6C12.31 0.21 11.16 0 10 0ZM17 0L13 4V5.5L10.45 8.05C10.3 8 10.15 8 10 8C9.46957 8 8.96086 8.21071 8.58579 8.58579C8.21071 8.96086 8 9.46957 8 10C8 10.5304 8.21071 11.0391 8.58579 11.4142C8.96086 11.7893 9.46957 12 10 12C10.5304 12 11.0391 11.7893 11.4142 11.4142C11.7893 11.0391 12 10.5304 12 10C12 9.85 12 9.7 11.95 9.55L14.5 7H16L20 3H17V0ZM10 4C8.4087 4 6.88258 4.63214 5.75736 5.75736C4.63214 6.88258 4 8.4087 4 10C4 11.5913 4.63214 13.1174 5.75736 14.2426C6.88258 15.3679 8.4087 16 10 16C11.5913 16 13.1174 15.3679 14.2426 14.2426C15.3679 13.1174 16 11.5913 16 10H14C14 11.0609 13.5786 12.0783 12.8284 12.8284C12.0783 13.5786 11.0609 14 10 14C8.93913 14 7.92172 13.5786 7.17157 12.8284C6.42143 12.0783 6 11.0609 6 10C6 8.93913 6.42143 7.92172 7.17157 7.17157C7.92172 6.42143 8.93913 6 10 6V4Z" />
    </svg>
  );
}

// MM_MEDIA_Diamond — verbatim SVG from MoMorph design (component 214:1817):
// faceted gem with internal cut lines.
export function DiamondIcon({ size = 24 }: { size?: number }) {
  return (
    <svg
      width={size}
      height={size}
      viewBox="0 0 20 20"
      fill={COLOR}
      xmlns="http://www.w3.org/2000/svg"
      aria-hidden
    >
      <path d="M14 7H17L12 14M8 7H12L10 15M3 7H6L8 14M13 2H15L17 5H14M9 2H11L12 5H8M5 2H7L6 5H3M4 0L0 6L10 20L20 6L16 0H4Z" />
    </svg>
  );
}

// MM_MEDIA_License — design glyph is an award-rosette (medal + ribbons).
// Phosphor v2.1.10 dropped the `License` export; `Medal` matches the visual.
export function LicenseIcon({ size = 24 }: { size?: number }) {
  return <Medal size={size} color={COLOR} weight="regular" aria-hidden />;
}
